...
const primaryNav = [
  ...
  { to: '/channels', label: 'Mes canaux', icon: Plug },
...
function formatRelativeTime(iso: string | null): string {
  ...
  const day = 24 * hour
  if (diffMs < hour) return `il y a ${Math.max(1, Math.round(diffMs / minute))} min`
  if (diffMs < day) return `il y a ${Math.round(diffMs / hour)} h`
...
function getAgentDisplayName(agent: AgentInfo | undefined, agentId: string): string {
  if (agentId === 'main') return 'Assistant principal'
...
function normalizeAgentList(items: AgentInfo[]): AgentInfo[] {
  return [...items]
    .map(agent => agent.id === 'main'
      ? { ...agent, name: 'Assistant principal', identity: { ...agent.identity, name: 'Assistant principal' } }
      : agent)
    .sort((a, b) => {
      if (a.id === 'main') return -1
      if (b.id === 'main') return 1
      return getAgentDisplayName(a, a.id).localeCompare(getAgentDisplayName(b, b.id), 'fr')
    })
}
...
function SessionStatusIndicator({ loading, unread }: { loading: boolean; unread: boolean }) {
  ...
  return (
    <span
      aria-label="L'IA a des réponses non lues"
      title="L'IA a des réponses non lues"
      ...
      >
        <nav className="space-y-1">
          {primaryNav.map(item => (
            <NavLink
              ...
              to={item.label === 'Nouvelle conversation' ? '/chat?new=1' : item.to}
              className={({ isActive }) =>
                `flex cursor-pointer items-center gap-3 rounded-xl px-2.5 py-2 text-sm transition-colors ${
                  isActive && item.label !== 'Nouvelle conversation'
        ...
        <div className="mt-7 min-h-0 flex-1 overflow-y-auto pr-1">
          <div className="mb-2 flex items-center justify-between px-2 text-xs text-slate-500">
            ...
            <IconButton
              label={allAgentsCollapsed ? 'Tout déplier' : 'Tout replier'}
          ...
          <div className={`sidebar-collapse ${agentsFolderOpen ? 'is-open' : ''}`}>
                <div>
                  {sessionsLoading ? (
                    <SidebarSessionSkeleton count={3} />
                      ...
                      <div className="px-2 py-3 text-xs text-slate-500">Aucune conversation d'agent</div>
                  ) : (
                ...
                <section key={group.id}>
                  <div className="group/agent flex items-center gap-1 rounded-xl px-2 py-1.5 text-sm text-light-text-secondary hover:bg-light-card/60">
                    ...
                    <IconButton
                      label={`Plus d'actions pour ${group.label}`}
                      onClick={event => {
                            ...
                            {isRenaming ? (
                              <div className="flex min-w-0 flex-1 items-center gap-1">
                                <ClearableInput
                                  ...
                                  autoFocus
                                  clearLabel="Effacer le titre"
                                />
                                <IconButton
                                  label="Enregistrer le titre"
                              ...
                              >
                                ...
                                <span className="ml-auto shrink-0 text-xs text-slate-400">
                                  {copiedKey === session.key ? 'Copié' : formatRelativeTime(session.updated_at)}
                                </span>
                              </button>
                            )}
                          </div>
            ...
            <section className="mt-5 border-t border-slate-200/70 pt-4">
              <div className="mb-2 flex items-center justify-between px-2 text-xs text-slate-500">
                ...
                <IconButton
                  label="Nouvelle conversation principale"
              ...
              <div className={`sidebar-collapse ${ordinaryFolderOpen ? 'is-open' : ''}`}>
                <div className="space-y-0.5">
                  {sessionsLoading ? (
                    ...
                    <div className="px-2 py-1 text-xs text-slate-400">Aucune conversation principale</div>
        ...
        <div className="mt-3 flex items-center justify-between rounded-xl px-2 py-2 text-slate-700">
          ...
          <div className="flex items-center gap-2">
            ...
            <Popconfirm
              ...
              description="L'état de connexion local sera effacé ; vous devrez vous reconnecter pour continuer."
              confirmText="Quitter"
              danger
              onConfirm={handleLogout}
            >
              <IconButton label="Se déconnecter" tone="danger" className="hover:bg-white/70">
                <LogOut size={16} />
              </IconButton>
            </Popconfirm>
          </div>
      ...
      {contextMenu && (
        ...
        >
          ...
          <Popconfirm
            ...
            description={`La session « ${getConfirmSessionTitle(contextMenu.session)} » sera supprimée ; cette action est irréversible.`}
