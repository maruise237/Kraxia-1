...
async function parseErrorMessage(res: Response): Promise<string> {
  const fallback = `Échec de la requête (${res.status})`
...
export async function createAgent(input: CreateAgentInput): Promise<CreateAgentResult> {
  ...
  if (!result) throw new Error('Échec de la création de l\'agent')
...
export async function generateAgentIcon(
  ...
  try {
    ...
    if (result.url?.startsWith('/api/')) {
      ...
      if (!imageRes.ok) throw new Error(`Échec du chargement de l'icône (${imageRes.status})`)
