...
const PROVIDER_CATALOG: ProviderInfo[] = [
  ...
  { id: 'dashscope', name: 'Qwen (DashScope)', icon: '☁️', placeholder: 'sk-...', defaultBaseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', defaultApi: 'openai-completions', defaultModel: 'qwen-max', requiresApiKey: true, category: 'cn' },
  { id: 'ark', name: 'ByteDance Ark / Doubao', icon: '🔥', placeholder: 'your-api-key', defaultBaseUrl: 'https://ark.cn-beijing.volces.com/api/v3', defaultApi: 'openai-completions', requiresApiKey: true, category: 'cn' },
  { id: 'zhipu', name: 'Zhipu GLM', icon: '💡', placeholder: 'your-api-key', defaultBaseUrl: 'https://open.bigmodel.cn/api/paas/v4', defaultApi: 'openai-completions', defaultModel: 'glm-4-plus', requiresApiKey: true, category: 'cn' },
  { id: 'ollama', name: 'Ollama (local)', icon: '🦙', defaultBaseUrl: 'http://localhost:11434/v1', defaultApi: 'openai-completions', placeholder: '', requiresApiKey: false, category: 'local' },
  { id: 'custom', name: 'API personnalisée', icon: '⚙️', placeholder: 'API key...', defaultApi: 'openai-completions', requiresApiKey: true, category: 'custom' },
]
...
export default function AIModels() {
  ...
  const handleSave = async () => {
    ...
    if (validModels.length === 0) { setError('Ajoutez au moins un modèle'); return }
    ...
    try {
      ...
      setShowForm(false)
      flash('Configuration du fournisseur enregistrée')
      reload()
    } catch (err: any) {
  ...
  const handleDeleteProvider = async (providerName: string) => {
    if (!confirm(`Confirmer la suppression du fournisseur "${providerName}" et de toute sa configuration de modèles ?`)) return
    setSaving(true)
    setError('')
    try {
      const newProviders = { ...configuredProviders }
      ...
      await updateModelsConfig({ providers: newProviders })
      flash(`Fournisseur ${providerName} supprimé`)
      reload()
    } catch (err: any) {
    ...
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-dark-text-secondary">
            Configurez vos fournisseurs de modèles IA
            {configuredModel && (
              <span className="ml-2">
                · Par défaut : <code className="rounded bg-dark-card px-1.5 py-0.5 text-xs text-accent-blue">{configuredModel}</code>
              </span>
            )}
          </p>
        </div>
      ...
      {hasProviders ? (
        ...
        <div className="mb-8 rounded-xl border border-dark-border bg-dark-card px-4 py-16 text-center">
          ...
          <p className="text-sm text-dark-text-secondary mb-3">Aucun fournisseur de modèles configuré</p>
          <button
            onClick={() => setShowPicker(true)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="rounded-xl bg-dark-card border border-dark-border max-w-lg w-full mx-4 shadow-xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border shrink-0">
              <h3 className="text-base font-semibold text-dark-text">Choisir un fournisseur</h3>
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              {configuredProviders[info.id] && (
                                <div className="text-[10px] text-accent-green">Déjà ajouté (peut être ajouté à nouveau)</div>
                              )}
                            </div>
                          </button>
                        ))}
          ...
          <div className="rounded-xl bg-dark-card border border-dark-border max-w-lg w-full mx-4 shadow-xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border shrink-0">
              <h3 className="text-base font-semibold text-dark-text">
                {editingProvider ? `Modifier : ${editingProvider}` : 'Configurer le fournisseur'}
              </h3>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {/* Name */}
              <div>
                <label className="mb-1 block text-xs font-medium text-dark-text-secondary">Nom du fournisseur *</label>
                <input
                  type="text"
                  value={form.name}
                  onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="Exemple : openai, anthropic"
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-xs font-medium text-dark-text-secondary">Liste des modèles *</label>
                  ...
                  {form.models.map((m, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="ID du modèle, ex. gpt-4o"
