...
export default function SystemSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      flash('Paramètres enregistrés — cliquez sur « Redémarrer la passerelle » pour les appliquer')
    } catch (err: any) {
  ...
  const handleRestart = async () => {
    if (!confirm('Confirmer le redémarrage de la passerelle ? Le service sera brièvement indisponible.')) return
    setRestarting(true)
    setError('')
    setConfigError('')
    try {
      ...
      const msg = err?.message || 'Échec du redémarrage'
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-dark-text-secondary">
            Gérez la configuration de la passerelle OpenClaw
          </p>
        </div>
        <button
          onClick={loadData}
      ...
      {configError && (
        <div className="mb-4 rounded-lg bg-accent-red/10 p-4 text-sm text-accent-red">
          <div className="flex items-center gap-2 mb-2 font-medium">
            <AlertCircle size={16} />
            La vérification de la configuration a échoué — corrigez-la avant de redémarrer la passerelle
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <section className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
          <div className="px-5 py-3 border-b border-dark-border flex items-center justify-between">
            ...
            >
              ...
              {restarting ? 'Redémarrage...' : 'Redémarrer la passerelle'}
            </button>
          </div>
          <div className="px-5 py-4">
            <div className="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-sm">
              <span className="text-dark-text-secondary">État de la connexion</span>
              <span className="flex items-center gap-1.5">
                ...
                <span className={gatewayConnected ? 'text-accent-green' : 'text-accent-red'}>
                  {gatewayConnected ? 'Connecté' : 'Non connecté'}
                </span>
              </span>

              <span className="text-dark-text-secondary">Fichier de configuration</span>
              <span className="text-dark-text font-mono text-xs">{String(status?.config_path || '-')}</span>

              <span className="text-dark-text-secondary">Espace de travail</span>
        ...
        <section className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
          <div className="px-5 py-3 border-b border-dark-border flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-dark-text">Informations du conteneur</h2>
            </div>
            <button
              onClick={async () => {
                if (!confirm('Confirmer le lancement de la réparation ? Cela corrigera automatiquement les problèmes de configuration et redémarrera la passerelle.')) return
                ...
                try {
                  ...
                  if (result.exit_code === 0) {
                    flash('Réparation terminée, le conteneur a redémarré automatiquement')
                    // Reload data after a short delay to reflect new status
                    setTimeout(() => loadData(), 5000)
                  } else {
                    setError('La commande de réparation a renvoyé un code de sortie non nul — consultez les détails ci-dessous')
                  }
                } catch (err: any) {
              ...
              title="Exécuter openclaw doctor --fix pour réparer la configuration"
            >
              <Wrench size={13} className={doctorRunning ? 'animate-spin' : ''} />
              {doctorRunning ? 'Réparation...' : 'Réparation en 1 clic'}
            </button>
          </div>
          <div className="px-5 py-4">
            {containerInfo?.container_name ? (
              <>
                <div className="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-sm">
                  <span className="text-dark-text-secondary">Nom du conteneur</span>
                  <span className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        flash('Nom du conteneur copié')
                      }}
                      className="text-dark-text-secondary hover:text-dark-text transition-colors"
                      title="Copier le nom du conteneur"
                    >
                      <Copy size={12} />
                    </button>
                  </span>
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    <span className={
                      ...
                      {containerInfo.status === 'running' ? 'En cours d\'exécution' :
                  ...
                  <span className="text-dark-text text-xs">
                    {containerInfo.created_at ? new Date(containerInfo.created_at).toLocaleString('fr-FR') : '-'}
                  </span>
                </div>
                ...
                {containerInfo.ports && containerInfo.ports.filter(function(p) { return p.host_port; }).length > 0 && (
                  <div className="mt-3 rounded-lg bg-dark-bg p-3 border border-dark-border">
                    <span className="text-xs font-medium text-dark-text-secondary">Mappage des ports</span>
              ...
              <div className="mt-4 rounded-lg bg-dark-bg p-3 border border-dark-border">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-dark-text">
                    Résultat de la réparation (exit code : {doctorResult.exit_code})
                  </span>
                </div>
                <pre className="text-xs text-dark-text-secondary font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
                  {doctorResult.stdout || doctorResult.stderr || '(aucune sortie)'}
                </pre>
              </div>
            )}

            <p className="mt-3 text-[11px] text-dark-text-secondary">
              En cas de problème, communiquez le nom du conteneur à l'administrateur pour faciliter le diagnostic. Si la configuration du conteneur est corrompue et empêche son démarrage, cliquez sur « Réparation en 1 clic ».
            </p>
          </div>
        </section>
        ...
        <section className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
          <div className="px-5 py-3 border-b border-dark-border flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-dark-text">Configuration de la passerelle</h2>
          </div>
          <div className="px-5 py-4 space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-dark-text-secondary">
                Une URL par ligne — contrôle l'accès cross-origin de la Control UI
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
          ...
          <div className="px-5 py-4">
            <div className="grid grid-cols-[140px_1fr] gap-x-4 gap-y-2.5 text-sm">
              <span className="text-dark-text-secondary">Version de la plateforme</span>
        ...
        <div className="flex gap-3 pb-6">
          ...
          <p className="text-xs text-dark-text-secondary self-center">
            Après enregistrement, cliquez sur « Redémarrer la passerelle » ci-dessus pour appliquer la configuration
          </p>
        </div>
      </div>
    </div>
