    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-dark-text-secondary">
          Gérez les fichiers de connaissances de chaque agent : documents, PDF, fichiers de données, etc.
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-4">
        <label className="text-sm font-medium text-dark-text">Choisir un agent :</label>
        {agentsLoading ? (
          <Loader2 size={16} className="animate-spin text-accent-blue" />
        ) : agents.length === 0 ? (
          <span className="text-sm text-dark-text-secondary">Aucun agent</span>
        ) : (
            ...
            >
              {agents.map(a => (
                <option key={a.id} value={a.id}>
                  {a.name || a.id}{a.id === 'main' ? ' (par défaut)' : ''}
                </option>
              ))}
            </select>
            <button
      ...
      {selectedAgent && (
        <>
          ...
          <div className="mb-4 rounded-lg bg-accent-blue/5 border border-accent-blue/10 px-4 py-2.5 text-xs text-dark-text-secondary">
            Chemin de la base de connaissances : <code className="rounded bg-dark-bg px-1.5 py-0.5 text-accent-blue">/opt/data/{knowledgeRoot(selectedAgent)}{subPath ? `/${subPath}` : ''}</code>
            <span className="ml-2">— l'agent peut référencer les fichiers de ce répertoire dans ses conversations</span>
          </div>
            ...
            <div className="rounded-xl border border-dark-border bg-dark-card overflow-hidden">
              ...
              {data?.items && data.items.length > 0 ? (
                ...
                <div className="px-4 py-12 text-center text-sm text-dark-text-secondary">
                  ...
                  <p>Base de connaissances vide</p>
                  <p className="mt-1 text-xs">Téléversez des documents, PDF, fichiers de données… l'agent pourra les référencer dans ses conversations</p>
                </div>
              )}
            </div>
          )}
