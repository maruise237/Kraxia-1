...
export default function Chat() {
  ...
  const loadSession = async (key: string) => {
    ...
    try {
      ...
      setError(err?.message || 'Échec du chargement de la session')
      setMessages([])
    } finally {
  ...
  const handleSaveRename = useCallback(async (key: string) => {
    ...
    try {
      ...
      setError(err?.message || 'Échec du renommage')
      setRenameSaving(false)
    }
  ...
  const streamRunEvents = useCallback((key: string, runId: string) => {
    return new Promise<void>((resolve, reject) => {
        ...
        try {
          ...
          if (eventType === 'run.failed') {
            finish(() => {
              ...
              setAgentRunning(false)
              sseCompletedRef.current = true
              reject(new Error(String(payload.error || 'Échec de l\'exécution')))
            })
          }
        } catch {
      ...
      sse.onerror = () => {
        finish(() => {
          reject(new Error('Flux d\'événements déconnecté'))
        })
      }
    })
                      ...
                      <div className="flex-1 min-w-0">
                        {isRenaming ? (
                          <input
                            ...
                            placeholder="Saisissez le titre de la session"
                            autoFocus
                          />
                        ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {activeSessionKey ? (
              ...
              <div className="flex items-center gap-2">
                {(sending || agentRunning) && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accent-blue/10 border border-accent-blue/20 animate-pulse">
                    ...
                    <span className="text-[11px] text-accent-blue font-medium">En traitement</span>
                  </div>
                )}
                <button
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {chatLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-dark-text-secondary">
                  ...
                  <p className="text-xs mt-1 opacity-60">Vous pouvez joindre des images et des fichiers</p>
                </div>
              ) : (
                        ...
                        <div className="rounded-xl px-4 py-2.5 bg-dark-card border border-dark-border">
                          {displayedText ? (
                            ...
                            <div className="flex items-center gap-2 text-sm text-dark-text-secondary">
                              ...
                              {liveToolCalls.some(t => t.status === 'running') ? 'Exécution des outils...' : 'Réflexion en cours...'}
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="px-5 py-3 border-t border-dark-border shrink-0">
              <div className="max-w-3xl mx-auto flex items-end gap-2">
                <button
                  ...
                  title="Joindre une pièce jointe (image/fichier)"
                >
                  <Paperclip size={16} />
                </button>
                <input
                ...
                <div className="relative flex-1">
                    ...
                    >
                      {filteredSlashCommands.length === 0 ? (
                        <div className="px-4 py-3 text-sm text-dark-text-secondary">
                          Aucune commande slash correspondante
                        </div>
                      ) : (
                  ...
                  <textarea
                    ...
                    onPaste={handlePaste}
                    placeholder={pendingFiles.length > 0 ? 'Ajoutez une note (optionnel)...' : 'Saisissez un message, collez une image, tapez / pour les commandes...'}
                    rows={1}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-dark-text-secondary">
            ...
            <p className="text-sm mb-4">Sélectionnez une session ou créez-en une nouvelle</p>
            <button
              onClick={handleNewSession}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="rounded-xl bg-dark-card border border-dark-border max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border">
              <h3 className="text-base font-semibold text-dark-text">Choisir un agent</h3>
              <button
