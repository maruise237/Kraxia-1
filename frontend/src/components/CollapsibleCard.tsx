...
function toolDisplayName(name: string): string {
  switch (name) {
    case 'skill_view': return 'Chargement de la compétence'
    case 'terminal': return 'Exécution de commande'
    case 'run_command': return 'Exécution de commande'
    case 'view_file': return 'Lecture du fichier'
    case 'write_file': return 'Écriture du fichier'
    case 'edit_file': return 'Modification du fichier'
...
function toolResultSummary(content: string): { status: 'success' | 'error' | 'neutral'; summary: string } {
  try {
    ...
    // view_file result
    if (parsed.content) {
      return { status: 'success', summary: 'Contenu du fichier lu' }
    }
      ...
      >
        ...
        <Wrench size={12} className="text-accent-blue shrink-0" />
          ...
          {group.assistant.content?.trim()
            ...
            : `Utilisation de l'outil : ${label}`}
        </span>
        <span className="ml-auto text-[10px] text-dark-text-secondary/60 shrink-0">
          {toolCalls.length} appel{toolCalls.length > 1 ? 's' : ''}
        </span>
      </button>
      {expanded && (
...
function formatResultContent(content: string): string {
  try {
    const parsed = JSON.parse(content)
    if (parsed.output !== undefined) return parsed.output || '(sortie vide)'
    ...
    <div className="my-1.5 max-w-3xl space-y-1">
      {tools.map(tc => (
        ...
        >
          ...
          {tc.status === 'running' && (
            <span className="ml-auto text-[10px] text-accent-blue/70">En cours...</span>
          )}
