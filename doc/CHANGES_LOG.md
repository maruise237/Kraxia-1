...
# SSO登录后，不管用户的容器是否存在和启动，都删掉重建，这个问题进行修复，确保只有在SSO的TOKEN变化时，才进行重建用户容器
⏺ 修复完成。改动在 platform/app/routes/auth.py:211-231：

  问题：每次 SSO 登录都无条件调用 destroy_container()，导致容器被销毁后重建，浪费用户时间。

  修复：
  1. 在更新用户信息之前，先读取旧的 sso_token
  2. 只有当 sso_token 发生变化时，才销毁容器（因为需要重新注入 INFOX_MED_TOKEN 环境变量）
  3. 如果 token 没变（比如短时间内重复登录），直接复用已有容器

  效果：
  - 新用户首次登录：无容器存在，ensure_running 会自动创建 → 行为不变
  - 老用户 token 未变（重复登录/刷新页面）：直接复用容器 → 秒级响应
  - 老用户 token 变化（新的 SSO 会话）：销毁重建 → 行为不变，但这是必要的
